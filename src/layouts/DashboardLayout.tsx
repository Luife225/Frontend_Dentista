...
type ModuleId = 'dashboard' | 'agenda' | 'pacientes' | 'teleodontologia' | 'notificaciones' | 'inventario' | 'caja' | 'consultorio'

interface NavItem { id: ModuleId; label: string; iconPath: string; group: string; path: string }
...
const ALL_NAV: NavItem[] = [
    { id: 'dashboard', label: 'Dashboard', group: 'Principal', path: '/dashboard', iconPath: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
    { id: 'agenda', label: 'Agenda', group: 'Principal', path: '/dashboard/agenda', iconPath: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
    { id: 'pacientes', label: 'Pacientes', group: 'Principal', path: '/dashboard/pacientes', iconPath: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
    { id: 'teleodontologia', label: 'Teleodontología', group: 'Principal', path: '/dashboard/teleodontologia', iconPath: 'M15 10l4.553-2.069A1 1 0 0121 8.82v6.36a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z' },
    { id: 'notificaciones', label: 'Notificaciones', group: 'Gestión', path: '/dashboard/notificaciones', iconPath: 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9' },
    { id: 'inventario', label: 'Inventario', group: 'Gestión', path: '/dashboard/inventario', iconPath: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
    { id: 'caja', label: 'Caja y reportes', group: 'Gestión', path: '/dashboard/caja', iconPath: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
    { id: 'consultorio', label: 'Configuración', group: 'Sistema', path: '/dashboard/consultorio', iconPath: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
]
...
export default function DashboardLayout() {
    ...
    if (!role || role === 'SUPER_ADMIN' || role === 'PACIENTE') return null
    ...
    const currentModuleId = navItems.find(n => n.path === location.pathname)?.id || 'dashboard'

    return (
        <div className="flex h-screen overflow-hidden">
            {/* Sidebar */}
            <aside className={`${collapsed ? 'w-14' : 'w-56'} shrink-0 flex flex-col h-full transition-all duration-200`} style={{ backgroundColor: '#0B3D3A' }}>
                {/* Logo + role badge */}
                                ...
                                {navItems.filter(n => n.group === group).map(item => {
                                    ...
                                    return (
                                        <button key={item.id}
                                            ...
                                            className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm transition-all relative ${isActive ? `${accentBgLight} ${accentText}` : 'text-white/45 hover:text-white/80 hover:bg-white/5'
                ...
                <div className={`p-3 border-t border-white/5 flex ${collapsed ? 'justify-center' : 'items-center gap-2'}`}>
                    {!collapsed && (
                        <>
                            ...
                            <button onClick={handleLogout} className="text-white/25 hover:text-white/60 transition-colors text-xs" title="Cerrar sesión">⏏</button>
                        </>
                    )}
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <header className="h-12 bg-white border-b border-slate-100 flex items-center px-5 gap-4 shrink-0">
                    <h2 className="text-sm font-semibold text-slate-700" style={{ fontFamily: 'Outfit' }}>{TITLES[currentModuleId]}</h2>
                    {isReadOnly && (
