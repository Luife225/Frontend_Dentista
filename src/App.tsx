...
// Modules each role can access — feedback #5: Inventario removed from Odontólogo
...
const ROLE_LABELS: Record<Role, string> = {
  ODONTOLOGO:'Odontólogo', RECEPCIONISTA:'Recepcionista', ADMIN_CLINICA:'Administrador', PACIENTE:'Paciente',
}
...
export default function App() {
  ...
  if (role === 'PACIENTE') return <PatientApp onLogout={() => setRole(null)} />
        ...
        <div className={`p-3 border-t border-white/5 flex ${collapsed?'justify-center':'items-center gap-2'}`}>
          {!collapsed && (
            <>
              ...
              <button onClick={() => setRole(null)} className="text-white/25 hover:text-white/60 transition-colors text-xs" title="Cerrar sesión">⏏</button>
            </>
          )}
